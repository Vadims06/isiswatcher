...
class IPTABLES_FORWARD_FOR_REMOTE_NETWORK_DEVICE_EXIST:
    ...
    def check(network_device_ip):
        ...
        for filter_table_row in iptc.easy.dump_chain('filter', 'FORWARD', ipv6=False):
            if filter_table_row.get('src', '') != IPTABLE_ENTRY_IP(network_device_ip):
                continue
            existed_records_ll.append(filter_table_row)
        if len(existed_records_ll) == 0:
            log.info("Iptables -t filter doesn't have settings for such remote network device. Good to add it.")
            return True
        log.critical( IPTABLES_FORWARD_FOR_REMOTE_NETWORK_DEVICE_EXIST.ASSERT_MSG +
            f"""Watcher's host has already {len(existed_records_ll)} filter records for {network_device_ip}.
            To remove them, run `sudo iptables -nvL -t filter --line-numbers` and sudo iptables -t filter -D FORWARD <num>"""
        )
        return False
