...
class WATCHER_CONFIG:
    ...
    def create_folder_with_settings(self):
        ...
        if self.enable_xdp:
            ...
            filtered_stages_wo_xdp = [d for d in watcher_config_yml['topology']['nodes']['h2']['stages']['create']['wait-for'] if d.get("node") != self.ISIS_FILTER_NODE_NAME]
            if not filtered_stages_wo_xdp:
                del watcher_config_yml['topology']['nodes']['h2']['stages']
            else:
                watcher_config_yml['topology']['nodes']['h2']['stages']['create']['wait-for'] = filtered_stages_wo_xdp
...
+---------------------------+                                        
    ...
    def disable_xdp(self):
        ...
        filtered_stages_wo_xdp = [d for d in current_clab_config['topology']['nodes']['h2']['stages']['create']['wait-for'] if d.get("node") != self.ISIS_FILTER_NODE_NAME]
        if not filtered_stages_wo_xdp:
            del current_clab_config['topology']['nodes']['h2']['stages']
        else:
            current_clab_config['topology']['nodes']['h2']['stages']['create']['wait-for'] = filtered_stages_wo_xdp
        self._do_save_watcher_config_file(current_clab_config)
        print("XDP disabled")
